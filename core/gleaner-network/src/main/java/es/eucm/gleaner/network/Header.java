...
public interface Header {
	 ...
	 * when sending data to the collector server. It is also used as the second
	 * layer of authentication (gleaner). In order to send traces to the
	 * collector server (gleaner) you must send an authorization token in every
	 * request. This authentication token is sent by the server when we issue a
